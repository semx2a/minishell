...
/*   Updated: 2022/10/11 16:18:16 by seozcan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{		
	...
	i = 0;
	o->fd_pipe = xmalloc(sizeof(int *) * (size_t)(o->pipe_nb * 2));
	if (!o->fd_pipe)
		ft_error();
	while (i <= o->cmd_nb - 1)
	{
...
void	pipes(t_obj *o)
{
	if (o->index == 0)
	{
		if (o->tokens->head->type == INFILE_ID)
			ft_dup2(o->fd_in, o->fd_pipe[1]);
		else
			ft_dup2(0, o->fd_pipe[1]);
	}
	else if (o->index == o->cmd_nb - 1 && o->tokens->head->type == OUTFILE_ID)
